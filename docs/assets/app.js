var ce=Object.defineProperty;var me=(e,t)=>{for(var a in t)ce(e,a,{get:t[a],enumerable:!0})};var T=class{routes=new Map;currentView=null;container;getCtx;categories=[];constructor(t,a){this.container=t,this.getCtx=a,window.addEventListener("hashchange",()=>this.handleRoute())}setCategories(t){this.categories=t}addRoute(t){this.routes.set(t.id,t)}handleRoute(){let a=(window.location.hash||"#/").slice(2);this.currentView&&this.currentView.unmount&&this.currentView.unmount(),this.container.innerHTML="";let r=this.routes.get(a);if(r){this.currentView=r;let n=document.createElement("button");n.className="back-button",n.textContent="Wszystkie kategorie",n.onclick=()=>{window.location.hash="#/"},this.container.appendChild(n);let o=document.createElement("div");o.className="category-content",o.id="current-category",this.container.appendChild(o),r.mount(o,this.getCtx())}else this.renderHome()}renderHome(){this.container.innerHTML=`
    ...
    `;let a=e.querySelector("#addSampleBtn"),r=e.querySelector("#sampleQty");a?.addEventListener("click",()=>{let n=parseInt(r.value)||1,o=n*10;t.cart.addItem({categoryId:"sample",categoryName:"Sample Category",details:{qty:n},price:o}),alert(`Dodano do koszyka: ${n} szt. za ${f(o)}`)})},unmount:()=>{console.log("Unmounting sample category")}};function de(e,t){let a=[...e].sort((o,l)=>o.min-l.min),r=a.find(o=>t>=o.min&&(o.max===null||t<=o.max));if(r)return r;let n=a.find(o=>o.min>=t);return n||a[a.length-1]}function ue(e,t){if(!t)return e;let a=t.find(r=>r.type==="minimum"&&r.unit==="m2");return a&&e<a.value?a.value:e}function k(e,t,a=[]){let r=ue(t,e.rules),n=de(e.tiers,r),o=0;e.pricing==="per_unit"?o=r*n.price:o=n.price;let l=0,c=[];if(e.modifiers)for(let i of a){let s=e.modifiers.find(d=>d.id===i);s&&(c.push(s.name),s.type==="percent"?l+=o*s.value:s.type==="fixed_per_unit"?l+=s.value*r:l+=s.value)}let p=o+l,m=e.rules?.find(i=>i.type==="minimum"&&i.unit==="pln");return m&&p<m.value&&(p=m.value),{basePrice:o,effectiveQuantity:r,tierPrice:n.price,modifiersTotal:l,totalPrice:parseFloat(p.toFixed(2)),appliedModifiers:c}}var I={id:"solwent-plakaty-200g",title:"SOLWENT - PLAKATY (Papier 200g po\u0142ysk)",unit:"m2",pricing:"per_unit",tiers:[{min:0,max:3,price:70},{min:3,max:9,price:65},{min:9,max:20,price:59},{min:20,max:40,price:53},{min:40,max:null,price:45}],rules:[{type:"minimum",unit:"m2",value:1}],modifiers:[{id:"EXPRESS",type:"percent",value:20}]};var R={id:"solwent-plakaty",name:"Solwent - Plakaty",mount:(e,t)=>{let a=I;e.innerHTML=`
      <div class="category-view">
        <h2>${a.title}</h2>
    ...
    `;let r=e.querySelector("#plakatyQty"),n=e.querySelector("#plakatyExpress"),o=e.querySelector("#plakatyResult"),l=e.querySelector("#addPlakatyBtn");function c(){let p=parseFloat(r.value)||0,m=n.checked?["EXPRESS"]:[];try{let i=k(p,a,m);o.textContent=f(i.totalPrice)}catch{o.textContent="B\u0142\u0105d"}}r.addEventListener("input",c),n.addEventListener("change",c),l.addEventListener("click",()=>{let p=parseFloat(r.value)||0,m=n.checked?["EXPRESS"]:[],i=k(p,a,m);t.cart.addItem({categoryId:a.id,categoryName:a.title,details:{qty:`${p} m2`,express:n.checked},price:i.totalPrice})}),c()}};var $={id:"vouchery",name:"Vouchery"};var v={name:"DYPLOMY - druk cyfrowy",modifiers:{satin:.12,express:.2,bulkDiscount:.12,bulkDiscountThreshold:6},formats:{DL:{name:"DL (99x210mm)",single:{"1":20,"2":30,"3":32,"4":34,"5":35,"6":35,"7":36,"8":37,"9":39,"10":40,"15":45,"20":49,"30":58,"40":65,"50":75,"100":120},double:{"1":20,"2":30,"3":32,"4":34,"5":35,"6":35,"7":36,"8":37,"9":39,"10":40,"15":45,"20":49,"30":58,"40":65,"50":75,"100":120}}}};function _(e){let{qty:t,sides:a,isSatin:r,express:n}=e,c=v.formats.DL[a===1?"single":"double"],p=Object.keys(c).map(Number).sort((y,w)=>y-w),m=p[0];for(let y of p)t>=y&&(m=y);let i=c[m.toString()],s=[];t>=v.modifiers.bulkDiscountThreshold&&s.push({id:"bulk-discount",name:`Rabat -${v.modifiers.bulkDiscount*100}% (od ${v.modifiers.bulkDiscountThreshold} szt)`,type:"percentage",value:-v.modifiers.bulkDiscount}),r&&s.push({id:"satin",name:"Papier satynowy (+12%)",type:"percentage",value:v.modifiers.satin}),n&&s.push({id:"express",name:"EXPRESS (+20%)",type:"percentage",value:v.modifiers.express});let d=0,u=[];for(let y of s)(y.type==="percent"||y.type==="percentage")&&(d+=i*y.value,u.push(y.name));let g=i+d;return{basePrice:i,effectiveQuantity:t,tierPrice:i/t,modifiersTotal:d,totalPrice:Math.round(g*100)/100,appliedModifiers:u}}var V={id:"dyplomy",name:"Dyplomy",async mount(e,t){let a=await fetch("categories/dyplomy.html");e.innerHTML=await a.text();let r=e.querySelector("#dypSides"),n=e.querySelector("#dypQty"),o=e.querySelector("#dypSatin"),l=e.querySelector("#calcBtn"),c=e.querySelector("#addToCartBtn"),p=e.querySelector("#dypResult"),m=()=>{let i={qty:parseInt(n.value)||1,sides:parseInt(r.value)||1,isSatin:o.checked,express:t.expressMode},s=_(i);return p.style.display="block",e.querySelector("#resUnitPrice").textContent=f(s.totalPrice/i.qty),e.querySelector("#resTotalPrice").textContent=f(s.totalPrice),e.querySelector("#resDiscountHint").style.display=s.appliedModifiers.includes("bulk-discount")?"block":"none",e.querySelector("#resExpressHint").style.display=i.express?"block":"none",e.querySelector("#resSatinHint").style.display=i.isSatin?"block":"none",t.updateLastCalculated(s.totalPrice,"Dyplomy"),{options:i,result:s}};l.addEventListener("click",()=>m()),c.addEventListener("click",()=>{let{options:i,result:s}=m();t.cart.addItem({id:`dyp-${Date.now()}`,category:"Dyplomy",name:`Dyplomy DL ${i.sides===1?"1-str":"2-str"}`,quantity:i.qty,unit:"szt",unitPrice:s.totalPrice/i.qty,isExpress:i.express,totalPrice:s.totalPrice,optionsHint:`${i.qty} szt, ${i.isSatin?"Satyna":"Kreda"}`,payload:i})}),m()}};function O(e,t){let a=Object.keys(e||{}).map(Number).filter(Number.isFinite).sort((n,o)=>n-o);if(!a.length)return null;let r=a.find(n=>t<=n);return r??null}var M={cyfrowe:{standardPrices:{"85x55":{noLam:{50:65,100:75,150:85,200:96,250:110,300:126,400:146,500:170,1e3:290},lam:{50:160,100:170,150:180,200:190,250:200,300:220,400:240,500:250,1e3:335}},"90x50":{noLam:{50:70,100:79,150:89,200:99,250:120,300:129,400:149,500:175,1e3:300},lam:{50:170,100:180,150:190,200:200,250:210,300:230,400:250,500:260,1e3:345}}},softtouchPrices:{"85x55":{noLam:{50:65,100:75,150:85,200:96,250:110,300:126,400:145,500:170,1e3:290},lam:{50:170,100:190,150:210,200:220,250:230,300:240,400:260,500:270,1e3:380}},"90x50":{noLam:{50:70,100:79,150:89,200:99,250:120,300:129,400:149,500:175,1e3:300},lam:{50:170,100:190,150:210,200:220,250:230,300:240,400:260,500:270,1e3:390}}},deluxe:{leadTime:"4\u20135 dni roboczych",options:{uv3d_softtouch:{label:"Maker UV 3D + folia SOFTTOUCH",prices:{50:280,100:320,200:395,250:479,400:655,500:778}},uv3d_gold_softtouch:{label:"Maker UV 3D + z\u0142ocenie + folia SOFTTOUCH",prices:{50:450,100:550,200:650,250:720,400:850,500:905}}}}}};function j(e){let t;e.family==="deluxe"?t=M.cyfrowe.deluxe.options[e.deluxeOpt].prices:t=(e.finish==="softtouch"?M.cyfrowe.softtouchPrices:M.cyfrowe.standardPrices)[e.size][e.lam];let a=O(t,e.qty);if(a==null)throw new Error("Brak progu cenowego dla takiej ilo\u015Bci.");let r=t[a];return{qtyBilled:a,total:r}}function F(e){let t=e.family||"standard",a=e.format||"85x55",r=e.folia==="none"?"noLam":"lam",n=e.finish||"mat",o=j({family:t,size:a,lam:r,finish:n,deluxeOpt:e.deluxeOpt,qty:e.qty}),l=o.total;return e.express&&(l=o.total*1.2),{totalPrice:parseFloat(l.toFixed(2)),basePrice:o.total,effectiveQuantity:e.qty,tierPrice:o.total/o.qtyBilled,modifiersTotal:e.express?o.total*.2:0,appliedModifiers:e.express?["TRYB EXPRESS"]:[],qtyBilled:o.qtyBilled}}var U={id:"wizytowki-druk-cyfrowy",name:"Wizyt\xF3wki - druk cyfrowy",mount(e,t){e.innerHTML=`
      <div class="category-view">
    ...
    `,this.initLogic(e,t)},initLogic(e,t){let a=e.querySelector("#w-family"),r=e.querySelector("#standard-options"),n=e.querySelector("#deluxe-options"),o=e.querySelector("#w-finish"),l=e.querySelector("#w-size"),c=e.querySelector("#w-lam"),p=e.querySelector("#w-deluxe-opt"),m=e.querySelector("#w-qty"),i=e.querySelector("#w-calculate"),s=e.querySelector("#w-add-to-cart"),d=e.querySelector("#w-result-display"),u=e.querySelector("#w-total-price"),g=e.querySelector("#w-billed-qty-hint"),y=e.querySelector("#w-express-hint");a.onchange=()=>{let x=a.value==="deluxe";r.style.display=x?"none":"block",n.style.display=x?"block":"none"};let w=null,h=null;i.onclick=()=>{h={family:a.value,finish:o.value,format:l.value,folia:c.value==="lam"?"matt_gloss":"none",deluxeOpt:p.value,qty:parseInt(m.value),express:t.expressMode};try{let x=F(h);w=x,u.innerText=f(x.totalPrice),g.innerText=`Rozliczono za: ${x.qtyBilled} szt.`,y.style.display=t.expressMode?"block":"none",d.style.display="block",s.disabled=!1,t.updateLastCalculated(x.totalPrice,"Wizyt\xF3wki")}catch(x){alert("B\u0142\u0105d: "+x.message)}},s.onclick=()=>{if(w&&h){let x=h.family==="deluxe"?"Wizyt\xF3wki DELUXE":"Wizyt\xF3wki Standard",z=h.express?", EXPRESS":"";t.cart.addItem({id:`wizytowki-${Date.now()}`,category:"Wizyt\xF3wki",name:x,quantity:w.qtyBilled,unit:"szt",unitPrice:w.totalPrice/w.qtyBilled,isExpress:h.express,totalPrice:w.totalPrice,optionsHint:`${h.qty} szt (rozliczono ${w.qtyBilled})${z}`,payload:w})}}}};var q={name:"Zaproszenia KREDA - druk cyfrowy",modifiers:{satin:.12,express:.2},formats:{A6:{name:"A6 (105x148mm)",single:{normal:{"10":30,"24":40,"32":45,"50":50,"75":60,"100":68,"150":79},folded:{"10":45,"24":55,"32":60,"50":71,"75":84,"100":99,"150":126}},double:{normal:{"10":35,"24":46,"32":57,"50":66,"75":79,"100":89,"150":115},folded:{"10":58,"24":66,"32":84,"50":105,"75":125,"100":149,"150":199}}},A5:{name:"A5 (148x210mm)",single:{normal:{"10":34,"24":42,"32":48,"50":55,"75":63,"100":79,"150":110},folded:{"10":55,"24":60,"32":75,"50":95,"75":125,"100":150,"150":199}},double:{normal:{"10":40,"24":49,"32":62,"50":79,"75":96,"100":119,"150":169},folded:{"10":65,"24":80,"32":115,"50":149,"75":190,"100":235,"150":325}}},DL:{name:"DL (99x210mm)",single:{normal:{"10":35,"24":50,"32":53,"50":59,"75":70,"100":81,"150":105},folded:{"10":45,"24":55,"32":63,"50":79,"75":97,"100":115,"150":149}},double:{normal:{"10":41,"24":55,"32":64,"50":74,"75":88,"100":105,"150":135},folded:{"10":65,"24":80,"32":90,"50":115,"75":150,"100":185,"150":245}}}}};function W(e){let{format:t,qty:a,sides:r,isFolded:n,isSatin:o,express:l}=e,c=q.formats[t];if(!c)throw new Error(`Invalid format: ${t}`);let p=r===1?"single":"double",m=n?"folded":"normal",i=c[p][m],s=Object.keys(i).map(Number).sort((x,z)=>x-z),d=s[0];for(let x of s)a>=x&&(d=x);let u=i[d.toString()],g=[];o&&g.push({id:"satin",name:"Papier satynowy (+12%)",type:"percentage",value:q.modifiers.satin}),l&&g.push({id:"express",name:"EXPRESS (+20%)",type:"percentage",value:q.modifiers.express});let y=0,w=[];for(let x of g)(x.type==="percent"||x.type==="percentage")&&(y+=u*x.value,w.push(x.name));let h=u+y;return{basePrice:u,effectiveQuantity:a,tierPrice:u/a,modifiersTotal:y,totalPrice:Math.round(h*100)/100,appliedModifiers:w}}var N={id:"zaproszenia-kreda",name:"Zaproszenia KREDA",async mount(e,t){let a=await fetch("categories/zaproszenia-kreda.html");e.innerHTML=await a.text();let r=e.querySelector("#zapFormat"),n=e.querySelector("#zapSides"),o=e.querySelector("#zapFolded"),l=e.querySelector("#zapQty"),c=e.querySelector("#zapSatin"),p=e.querySelector("#calcBtn"),m=e.querySelector("#addToCartBtn"),i=e.querySelector("#zapResult"),s=()=>{let d={format:r.value,qty:parseInt(l.value)||10,sides:parseInt(n.value)||1,isFolded:o.checked,isSatin:c.checked,express:t.expressMode},u=W(d);return i.style.display="block",e.querySelector("#resUnitPrice").textContent=f(u.totalPrice/d.qty),e.querySelector("#resTotalPrice").textContent=f(u.totalPrice),e.querySelector("#resExpressHint").style.display=d.express?"block":"none",e.querySelector("#resSatinHint").style.display=d.isSatin?"block":"none",t.updateLastCalculated(u.totalPrice,"Zaproszenia"),{options:d,result:u}};p.addEventListener("click",()=>s()),m.addEventListener("click",()=>{let{options:d,result:u}=s();t.cart.addItem({id:`zap-${Date.now()}`,category:"Zaproszenia Kreda",name:`Zaproszenia ${d.format} ${d.sides===1?"1-str":"2-str"}${d.isFolded?" sk\u0142adane":""}`,quantity:d.qty,unit:"szt",unitPrice:u.totalPrice/d.qty,isExpress:d.express,totalPrice:u.totalPrice,optionsHint:`${d.qty} szt, ${d.isSatin?"Satyna":"Kreda"}`,payload:d})}),s()}};var X={name:"Ulotki - Cyfrowe Dwustronne",formats:{A6:{name:"A6 (105x148)",tiers:[{min:10,max:10,price:40},{min:20,max:20,price:47},{min:30,max:30,price:49},{min:40,max:40,price:55},{min:50,max:50,price:60},{min:60,max:60,price:65},{min:70,max:70,price:70},{min:80,max:80,price:75},{min:90,max:90,price:90},{min:100,max:100,price:95},{min:150,max:150,price:120},{min:200,max:200,price:140},{min:300,max:300,price:190},{min:400,max:400,price:230},{min:500,max:500,price:270},{min:700,max:700,price:355},{min:1e3,max:1e3,price:476}]},A5:{name:"A5 (148 x 210)",tiers:[{min:10,max:10,price:45},{min:20,max:20,price:55},{min:30,max:30,price:60},{min:40,max:40,price:72},{min:50,max:50,price:90},{min:60,max:60,price:99},{min:70,max:70,price:112},{min:80,max:80,price:122},{min:90,max:90,price:130},{min:100,max:100,price:140},{min:150,max:150,price:180},{min:200,max:200,price:220},{min:300,max:300,price:310},{min:400,max:400,price:390},{min:500,max:500,price:460},{min:700,max:700,price:620},{min:1e3,max:1e3,price:790}]},DL:{name:"DL (99 x 210)",tiers:[{min:10,max:10,price:40},{min:20,max:20,price:50},{min:30,max:30,price:55},{min:40,max:40,price:65},{min:50,max:50,price:70},{min:60,max:60,price:78},{min:70,max:70,price:88},{min:80,max:80,price:99},{min:90,max:90,price:109},{min:100,max:100,price:119},{min:150,max:150,price:150},{min:200,max:200,price:179},{min:300,max:300,price:235},{min:400,max:400,price:290},{min:500,max:500,price:350},{min:700,max:700,price:460},{min:1e3,max:1e3,price:590}]}}};function ge(e){let t=X.formats[e];if(!t)throw new Error(`Invalid format: ${e}`);return{id:`ulotki-cyfrowe-dwustronne-${e.toLowerCase()}`,title:`Ulotki Cyfrowe Dwustronne ${t.name}`,unit:"szt",pricing:"flat",tiers:t.tiers,modifiers:[{id:"express",name:"TRYB EXPRESS",type:"percent",value:.2}]}}function K(e){let t=ge(e.format),a=[];return e.express&&a.push("express"),k(t,e.qty,a)}var J={id:"ulotki-cyfrowe-dwustronne",name:"Ulotki - Cyfrowe Dwustronne",async mount(e,t){try{let a=await fetch("categories/ulotki-cyfrowe-dwustronne.html");if(!a.ok)throw new Error("Failed to load template");e.innerHTML=await a.text(),this.initLogic(e,t)}catch(a){e.innerHTML=`<div class="error">B\u0142\u0105d \u0142adowania: ${a}</div>`}},initLogic(e,t){let a=e.querySelector("#u-format"),r=e.querySelector("#u-qty"),n=e.querySelector("#u-calculate"),o=e.querySelector("#u-add-to-cart"),l=e.querySelector("#u-result-display"),c=e.querySelector("#u-total-price"),p=e.querySelector("#u-express-hint"),m=null,i=null;n.onclick=()=>{i={format:a.value,qty:parseInt(r.value),express:t.expressMode};try{let s=K(i);m=s,c.innerText=f(s.totalPrice),p&&(p.style.display=t.expressMode?"block":"none"),l.style.display="block",o.disabled=!1,t.updateLastCalculated(s.totalPrice,"Ulotki")}catch(s){alert("B\u0142\u0105d: "+s.message)}},o.onclick=()=>{if(m&&i){let s=i.express?", EXPRESS":"";t.cart.addItem({id:`ulotki-dwustronne-${Date.now()}`,category:"Ulotki",name:`Ulotki Dwustronne ${i.format}`,quantity:i.qty,unit:"szt",unitPrice:m.totalPrice/i.qty,isExpress:i.express,totalPrice:m.totalPrice,optionsHint:`${i.qty} szt, Dwustronne${s}`,payload:m})}}}};var Z={name:"Ulotki - Cyfrowe Jednostronne",formats:{A6:{name:"A6 (105x148)",tiers:[{min:10,max:10,price:30},{min:20,max:20,price:35},{min:30,max:30,price:40},{min:40,max:40,price:45},{min:50,max:50,price:55},{min:60,max:60,price:61},{min:70,max:70,price:65},{min:80,max:80,price:70},{min:90,max:90,price:75},{min:100,max:100,price:79},{min:150,max:150,price:90},{min:200,max:200,price:110},{min:300,max:300,price:145},{min:400,max:400,price:160},{min:500,max:500,price:190},{min:700,max:700,price:255},{min:1e3,max:1e3,price:320}]},A5:{name:"A5 (148 x 210)",tiers:[{min:10,max:10,price:35},{min:20,max:20,price:42},{min:30,max:30,price:50},{min:40,max:40,price:59},{min:50,max:50,price:69},{min:60,max:60,price:75},{min:70,max:70,price:82},{min:80,max:80,price:88},{min:90,max:90,price:93},{min:100,max:100,price:95},{min:150,max:150,price:130},{min:200,max:200,price:150},{min:300,max:300,price:210},{min:400,max:400,price:259},{min:500,max:500,price:300},{min:700,max:700,price:410},{min:1e3,max:1e3,price:530}]},DL:{name:"DL (99 x 210)",tiers:[{min:10,max:10,price:30},{min:20,max:20,price:35},{min:30,max:30,price:40},{min:40,max:40,price:45},{min:50,max:50,price:55},{min:60,max:60,price:60},{min:70,max:70,price:65},{min:80,max:80,price:70},{min:90,max:90,price:75},{min:100,max:100,price:83},{min:150,max:150,price:100},{min:200,max:200,price:120},{min:300,max:300,price:160},{min:400,max:400,price:199},{min:500,max:500,price:230},{min:700,max:700,price:310},{min:1e3,max:1e3,price:399}]}}};function he(e){let t=Z.formats[e];if(!t)throw new Error(`Invalid format: ${e}`);return{id:`ulotki-cyfrowe-jednostronne-${e.toLowerCase()}`,title:`Ulotki Cyfrowe Jednostronne ${t.name}`,unit:"szt",pricing:"flat",tiers:t.tiers,modifiers:[{id:"express",name:"TRYB EXPRESS",type:"percent",value:.2}]}}function Q(e){let t=he(e.format),a=[];return e.express&&a.push("express"),k(t,e.qty,a)}var Y={id:"ulotki-cyfrowe-jednostronne",name:"Ulotki - Cyfrowe Jednostronne",async mount(e,t){try{let a=await fetch("categories/ulotki-cyfrowe-jednostronne.html");if(!a.ok)throw new Error("Failed to load template");e.innerHTML=await a.text(),this.initLogic(e,t)}catch(a){e.innerHTML=`<div class="error">B\u0142\u0105d \u0142adowania: ${a}</div>`}},initLogic(e,t){let a=e.querySelector("#uj-format"),r=e.querySelector("#uj-qty"),n=e.querySelector("#uj-calculate"),o=e.querySelector("#uj-add-to-cart"),l=e.querySelector("#uj-result-display"),c=e.querySelector("#uj-total-price"),p=e.querySelector("#uj-express-hint"),m=null,i=null;n.onclick=()=>{i={format:a.value,qty:parseInt(r.value),express:t.expressMode};try{let s=Q(i);m=s,c.innerText=f(s.totalPrice),p&&(p.style.display=t.expressMode?"block":"none"),l.style.display="block",o.disabled=!1,t.updateLastCalculated(s.totalPrice,"Ulotki")}catch(s){alert("B\u0142\u0105d: "+s.message)}},o.onclick=()=>{if(m&&i){let s=i.express?", EXPRESS":"";t.cart.addItem({id:`ulotki-jednostronne-${Date.now()}`,category:"Ulotki",name:`Ulotki Jednostronne ${i.format}`,quantity:i.qty,unit:"szt",unitPrice:m.totalPrice/i.qty,isExpress:i.express,totalPrice:m.totalPrice,optionsHint:`${i.qty} szt, Jednostronne${s}`,payload:m})}}}};var G={id:"banner",title:"Bannery",unit:"m2",pricing:"per_unit",materials:[{id:"powlekany",name:"Banner powlekany",tiers:[{min:1,max:25,price:53},{min:26,max:50,price:49},{min:51,max:null,price:45}]},{id:"blockout",name:"Banner Blockout",tiers:[{min:1,max:25,price:64},{min:26,max:50,price:59},{min:51,max:null,price:55}]}],modifiers:[{id:"oczkowanie",name:"Oczkowanie (+2.50 z\u0142/m2)",type:"fixed_per_unit",value:2.5},{id:"express",name:"TRYB EXPRESS (+20%)",type:"percent",value:.2}]};function ee(e){let t=G,a=t.materials.find(o=>o.id===e.material);if(!a)throw new Error(`Unknown material: ${e.material}`);let r={id:t.id,title:t.title,unit:t.unit,pricing:t.pricing,tiers:a.tiers,modifiers:t.modifiers},n=[];return e.oczkowanie&&n.push("oczkowanie"),e.express&&n.push("express"),k(r,e.areaM2,n)}var te={id:"banner",name:"Bannery",async mount(e,t){try{let a=await fetch("categories/banner.html");if(!a.ok)throw new Error("Failed to load template");e.innerHTML=await a.text(),this.initLogic(e,t)}catch(a){e.innerHTML=`<div class="error">B\u0142\u0105d \u0142adowania: ${a}</div>`}},initLogic(e,t){let a=e.querySelector("#b-material"),r=e.querySelector("#b-area"),n=e.querySelector("#b-oczkowanie"),o=e.querySelector("#b-calculate"),l=e.querySelector("#b-add-to-cart"),c=e.querySelector("#b-result-display"),p=e.querySelector("#b-unit-price"),m=e.querySelector("#b-total-price"),i=e.querySelector("#b-express-hint"),s=null,d=null;o.onclick=()=>{d={material:a.value,areaM2:parseFloat(r.value),oczkowanie:n.checked,express:t.expressMode};try{let u=ee(d);s=u,p.innerText=f(u.tierPrice),m.innerText=f(u.totalPrice),i&&(i.style.display=t.expressMode?"block":"none"),c.style.display="block",l.disabled=!1,t.updateLastCalculated(u.totalPrice,"Banner")}catch(u){alert("B\u0142\u0105d: "+u.message)}},l.onclick=()=>{if(s&&d){let u=a.options[a.selectedIndex].text,g=[`${d.areaM2} m2`,d.oczkowanie?"z oczkowaniem":"bez oczkowania",d.express?"EXPRESS":""].filter(Boolean).join(", ");t.cart.addItem({id:`banner-${Date.now()}`,category:"Bannery",name:u,quantity:d.areaM2,unit:"m2",unitPrice:s.tierPrice,isExpress:d.express,totalPrice:s.totalPrice,optionsHint:g,payload:s})}}}};var E={};me(E,{category:()=>ve,default:()=>Se,groups:()=>Ee,modifiers:()=>Le});var ve="Wlepki / Naklejki",Ee=[{id:"wlepki_obrys_folia",title:"Wlepki po obrysie (Folia Bia\u0142a/Trans)",unit:"m2",pricing:"per_unit",tiers:[{min:1,max:5,price:67},{min:6,max:25,price:60},{min:26,max:50,price:52},{min:51,max:null,price:48}],rules:[{type:"minimum",unit:"m2",value:1}]},{id:"wlepki_polipropylen",title:"Wlepki po obrysie - Polipropylen",unit:"m2",pricing:"per_unit",tiers:[{min:1,max:10,price:50},{min:11,max:null,price:42}],rules:[{type:"minimum",unit:"m2",value:1}]},{id:"wlepki_standard_folia",title:"Folia Bia\u0142a / Transparentna (standard)",unit:"m2",pricing:"per_unit",tiers:[{min:1,max:5,price:54},{min:6,max:25,price:50},{min:26,max:50,price:46},{min:51,max:null,price:42}],rules:[{type:"minimum",unit:"m2",value:1}]}],Le=[{id:"mocny_klej",name:"Mocny klej",type:"percent",value:.12},{id:"arkusze",name:"Ci\u0119te na arkusze",type:"fixed_per_unit",value:2},{id:"pojedyncze",name:"Ci\u0119te na pojedyncze sztuki",type:"fixed_per_unit",value:10},{id:"express",name:"EXPRESS",type:"percent",value:.2}],Se={category:ve,groups:Ee,modifiers:Le};function ae(e){let t=E,a=t.groups.find(o=>o.id===e.groupId);if(!a)throw new Error(`Unknown group: ${e.groupId}`);let r={id:"wlepki",title:a.title,unit:a.unit,pricing:a.pricing||"per_unit",tiers:a.tiers,modifiers:t.modifiers,rules:a.rules||[{type:"minimum",unit:"m2",value:1}]},n=[...e.modifiers];return e.express&&n.push("express"),k(r,e.area,n)}var re={id:"wlepki-naklejki",name:"Wlepki / Naklejki",async mount(e,t){let a=E;try{let u=await fetch("categories/wlepki-naklejki.html");if(!u.ok)throw new Error("Failed to load template");e.innerHTML=await u.text()}catch(u){e.innerHTML=`<div class="error">B\u0142\u0105d \u0142adowania szablonu: ${u}</div>`;return}let r=e.querySelector("#wlepki-group"),n=e.querySelector("#wlepki-area"),o=e.querySelector("#btn-calculate"),l=e.querySelector("#btn-add-to-cart"),c=e.querySelector("#wlepki-result"),p=e.querySelector("#unit-price"),m=e.querySelector("#total-price"),i=null,s=null,d=()=>{let u=e.querySelectorAll(".wlepki-mod:checked"),g=Array.from(u).map(y=>y.value);s={groupId:r.value,area:parseFloat(n.value)||0,express:t.expressMode,modifiers:g};try{let y=ae(s);i=y,p.textContent=f(y.tierPrice),m.textContent=f(y.totalPrice),c.style.display="block",l.disabled=!1,t.updateLastCalculated(y.totalPrice,"Wlepki")}catch(y){alert("B\u0142\u0105d: "+y.message)}};o.addEventListener("click",d),l.addEventListener("click",()=>{if(!i||!s)return;let u=a.groups.find(y=>y.id===s.groupId),g=s.modifiers.map(y=>{let w=a.modifiers.find(h=>h.id===y);return w?w.name:y});s.express&&g.unshift("EXPRESS (+20%)"),t.cart.addItem({id:`wlepki-${Date.now()}`,category:"Wlepki / Naklejki",name:u?.title||"Wlepki",quantity:s.area,unit:"m2",unitPrice:i.tierPrice,isExpress:!!s.express,totalPrice:i.totalPrice,optionsHint:g.join(", ")||"Standard",payload:i})})}};var H={name:"Roll-up Jednostronny",formats:{"85x200":{width:.85,height:2,tiers:[{min:1,max:5,price:290},{min:6,max:10,price:275}]},"100x200":{width:1,height:2,tiers:[{min:1,max:5,price:305},{min:6,max:10,price:285}]},"120x200":{width:1.2,height:2,tiers:[{min:1,max:5,price:330},{min:6,max:10,price:310}]},"150x200":{width:1.5,height:2,tiers:[{min:1,max:5,price:440},{min:6,max:10,price:425}]}},replacement:{labor:50,print_per_m2:80}};function oe(e){let t=H.formats[e.format];if(!t)throw new Error(`Unknown format: ${e.format}`);let a;if(e.isReplacement){let o=t.width*t.height*H.replacement.print_per_m2+H.replacement.labor;a={id:"roll-up-replacement",title:`Wymiana wk\u0142adu (${e.format})`,unit:"szt",pricing:"per_unit",tiers:[{min:1,max:null,price:o}],modifiers:[{id:"express",name:"EXPRESS",type:"percent",value:.2}]}}else a={id:"roll-up-full",title:`Roll-up Komplet (${e.format})`,unit:"szt",pricing:"per_unit",tiers:t.tiers,modifiers:[{id:"express",name:"EXPRESS",type:"percent",value:.2}]};let r=[];return e.express&&r.push("express"),k(a,e.qty,r)}var ie={id:"roll-up",name:"Roll-up",async mount(e,t){let a=await fetch("categories/roll-up.html");e.innerHTML=await a.text();let r=e.querySelector("#rollUpType"),n=e.querySelector("#rollUpFormat"),o=e.querySelector("#rollUpQty"),l=e.querySelector("#calcBtn"),c=e.querySelector("#addToCartBtn"),p=e.querySelector("#rollUpResult"),m=()=>{let i={format:n.value,qty:parseInt(o.value)||1,isReplacement:r.value==="replacement",express:t.expressMode},s=oe(i);return p.style.display="block",e.querySelector("#resUnitPrice").textContent=f(s.totalPrice/i.qty),e.querySelector("#resTotalPrice").textContent=f(s.totalPrice),e.querySelector("#resExpressHint").style.display=i.express?"block":"none",t.updateLastCalculated(s.totalPrice,"Roll-up"),{options:i,result:s}};l.addEventListener("click",()=>m()),c.addEventListener("click",()=>{let{options:i,result:s}=m();t.cart.addItem({id:`rollup-${Date.now()}`,category:"Roll-up",name:`${i.isReplacement?"Wymiana wk\u0142adu":"Roll-up Komplet"} ${i.format}`,quantity:i.qty,unit:"szt",unitPrice:s.totalPrice/i.qty,isExpress:i.express,totalPrice:s.totalPrice,optionsHint:`${i.format}, ${i.qty} szt`,payload:i})}),m()}};async function Ce(e){let t=await fetch(`./categories/${e}`);if(!t.ok)throw new Error(`Failed to load ${e}`);return t.text()}function L(e,t,a){return{id:e,name:t,mount:async(r,n)=>{r.innerHTML='<div style="padding: 20px; text-align: center; color: #999;">\u23F3 \u0141adowanie kategorii...</div>';try{let o=await Ce(a);r.innerHTML=o,Me(r,n)}catch(o){r.innerHTML=`
          <div style="padding: 40px; text-align: center; color: #e74c3c;">
            \u274C B\u0142\u0105d \u0142adowania kategorii: ${t}
            <br><small>${o}</small>
          </div>
        `,console.error("Category load error:",o)}}}}function Me(e,t){e.querySelectorAll("button[data-action]").forEach(r=>{let n=r.getAttribute("data-action");n==="calculate"&&r.addEventListener("click",()=>{console.log("Calculate clicked")}),n==="add-to-basket"&&r.addEventListener("click",()=>{t.addToBasket({category:e.getAttribute("data-category-id")||"unknown",price:parseFloat(e.getAttribute("data-price")||"0"),description:e.getAttribute("data-description")||""})})})}var qe=[{produkt:"A0+ 914\xD71292",jednostka:"1 szt",cena:26,baseWidth:914,baseLength:1292,typ:"cad_length"},{produkt:"A1+ 610\xD7914",jednostka:"1 szt",cena:18,baseWidth:610,baseLength:914,typ:"cad_length"},{produkt:"A2+ 450\xD7610",jednostka:"1 szt",cena:12,baseWidth:450,baseLength:610,typ:"cad_length"},{produkt:"MB 90cm",jednostka:"1 mb",cena:21,baseWidth:900,baseLength:1e3,typ:"cad_length"},{produkt:"MB 61cm",jednostka:"1 mb",cena:15,baseWidth:610,baseLength:1e3,typ:"cad_length"}],ne={id:"druk-cad",name:"\u{1F5FA}\uFE0F Druk CAD wielkoformatowy",mount:(e,t)=>{e.innerHTML=`
    ...
    `;let a=e.querySelector("#cad-products");a&&qe.forEach((r,n)=>{let o=document.createElement("div");o.className="product-card",o.innerHTML=`
      ...
      `,a.appendChild(o);let l=o.querySelector(`#clen-${n}`),c=o.querySelector(`#price-${n}`);l.addEventListener("input",()=>{let m=parseFloat(l.value)||r.baseLength,i=m/r.baseLength,s=r.cena*i;c&&(c.textContent=`${s.toFixed(2)} z\u0142`),t.updateLastCalculated(s,`${r.produkt} - ${m}mm`)}),o.querySelector(".btn-success")?.addEventListener("click",()=>{let m=parseFloat(l.value)||r.baseLength,i=m/r.baseLength,s=r.cena*i;t.addToBasket({category:"Druk CAD",price:s,description:`${r.produkt} - ${m}mm`}),alert(`\u2705 Dodano do listy: ${r.produkt} (${s.toFixed(2)} z\u0142)`)})})}};var le={id:"druk-a4-a3",name:"\u{1F4C4} Druk A4/A3",mount:(e,t)=>{e.innerHTML=`
      <div class="category-form">
        <h2>Druk A4/A3</h2>
    ...
    `;let a=0,r=e.querySelector("#calculate"),n=e.querySelector("#addToBasket"),o=e.querySelector("#total-price");r?.addEventListener("click",()=>{let l=e.querySelector("#format").value,c=parseInt(e.querySelector("#pages").value)||1,p=e.querySelector("#color").value,m=p==="mono"?.5:1.5,i=l==="A3"?1.5:1;a=c*m*i,o&&(o.textContent=`${a.toFixed(2)} z\u0142`),t.updateLastCalculated(a,`${l} ${p} - ${c} str.`)}),n?.addEventListener("click",()=>{if(a===0){alert("\u26A0\uFE0F Najpierw oblicz cen\u0119!");return}let l=e.querySelector("#format").value,c=e.querySelector("#pages").value,p=e.querySelector("#color").value;t.addToBasket({category:"Druk A4/A3",price:a,description:`Format: ${l}, Strony: ${c}, Kolor: ${p}`}),alert(`\u2705 Dodano do listy: ${a.toFixed(2)} z\u0142`)})}};var se=[B,R,$,V,U,N,J,Y,te,re,ie,ne,le,L("druk-a4-a3-skan","\u{1F4C4} Druk A4/A3 + skan","druk-a4-a3-skan.html"),L("cad-ops","\u{1F5FA}\uFE0F CAD OPS","cad-ops.html"),L("folia-szroniona","\u2728 Folia Szroniona","folia-szroniona.html"),L("laminowanie","\u{1F512} Laminowanie","laminowanie.html")];var b=new C;function S(){let e=document.getElementById("basket-items"),t=document.getElementById("basket-total"),a=document.getElementById("json-preview");if(!e||!t||!a)return;let r=b.getItems();if(r.length===0)e.innerHTML=`
      <p style="color: #999; text-align: center; padding: 20px;">
        Brak pozycji<br>
        <small>Kliknij \u201EDodaj\u201D, aby zbudowa\u0107 list\u0119.</small>
      </p>
    `,t.textContent="0,00 z\u0142";else{e.innerHTML=r.map((o,l)=>`
    ...
    `).join("");let n=b.getGrandTotal();t.innerText=f(n)}a.innerText=JSON.stringify(r.map(n=>n.payload),null,2)}window.removeItem=e=>{b.removeItem(e),S()};document.addEventListener("DOMContentLoaded",()=>{let e=document.getElementById("viewContainer"),t=document.getElementById("categorySelector"),a=document.getElementById("categorySearch"),r=document.getElementById("tryb-express");if(!e||!t||!r||!a)return;let n=()=>({cart:{addItem:l=>{b.addItem(l),S()}},addToBasket:l=>{b.addItem({id:`item-${Date.now()}`,category:l.category,name:l.description||"Produkt",quantity:1,unit:"szt.",unitPrice:l.price,isExpress:r.checked,totalPrice:l.price,optionsHint:l.description||"",payload:l}),S()},expressMode:r.checked,updateLastCalculated:(l,c)=>{let p=document.getElementById("last-calculated"),m=document.getElementById("currentHint");p&&(p.innerText=f(l)),m&&(m.innerText=c?`(${c})`:"")}}),o=new T(e,n);o.setCategories(A),se.forEach(l=>{o.addRoute(l)}),A.forEach(l=>{let c=document.createElement("option");c.value=l.id,c.innerText=`${l.icon} ${l.name}`,l.implemented||(c.disabled=!0,c.innerText+=" (wkr\xF3tce)"),t.appendChild(c)}),t.addEventListener("change",()=>{let l=t.value;l?window.location.hash=`#/${l}`:window.location.hash="#/"}),a.addEventListener("input",()=>{let l=a.value.toLowerCase();Array.from(t.options).forEach((p,m)=>{if(m===0)return;let i=p.text.toLowerCase();p.hidden=!i.includes(l)})}),a.addEventListener("keydown",l=>{if(l.key==="Enter"){let c=a.value.toLowerCase(),p=Array.from(t.options).find((m,i)=>i>0&&!m.hidden&&!m.disabled);p&&(t.value=p.value,window.location.hash=`#/${p.value}`,a.value="")}}),window.addEventListener("hashchange",()=>{let c=(window.location.hash||"#/").slice(2);t.value=c}),r.addEventListener("change",()=>{let l=window.location.hash;window.location.hash="",window.location.hash=l}),document.getElementById("clear-basket")?.addEventListener("click",()=>{b.clear(),S()}),document.getElementById("export-excel")?.addEventListener("click",()=>{let l={name:document.getElementById("client-name").value||"Anonim",phone:document.getElementById("client-phone").value||"-",email:document.getElementById("client-email").value||"-",priority:document.getElementById("priority").value};if(b.isEmpty()){alert("Lista jest pusta!");return}D(b.getItems(),l)}),document.getElementById("copy-json")?.addEventListener("click",()=>{let l=b.getItems(),c=JSON.stringify(l.map(p=>p.payload),null,2);navigator.clipboard.writeText(c).then(()=>{alert("JSON skopiowany do schowka!")})}),S(),o.start()});"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js").then(e=>console.log("SW registered",e)).catch(e=>console.error("SW failed",e))});
