...
function resolveTierPrice(format: FormatKey, qty: number): number {
  const tier = QTY_TIERS.find((t) => qty >= t.min && (t.max === null || qty <= t.max));
  if (!tier) return 0;
  const key = `broszury-katalogi-${format}-${tier.suffix}`;
  return resolveStoredPrice(key, 0);
}
  ...
  initLogic(container: HTMLElement, ctx: ViewContext) {
    ...
    const ensureLegend = () => {
      ...
      const rows = formats.map((fmt) => {
        const tierRows = QTY_TIERS.map((tier) => {
          const key = `broszury-katalogi-${fmt}-${tier.suffix}`;
          const price = resolveStoredPrice(key, 0);
          const rangeLabel = tier.max === null ? `${tier.min}+ szt.` : `${tier.min}–${tier.max} szt.`;
          return `<tr><td>${rangeLabel}</td><td>${price > 0 ? formatPLN(price) : "—"}</td></tr>`;
        }).join("");
