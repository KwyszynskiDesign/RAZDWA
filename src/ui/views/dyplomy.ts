  ...
  async mount(container, ctx) {
    ...
    const calculate = () => {
      ...
      const breakdown = [
        ...
        `<div><strong>Cena  (${options.sides === 1 ? "jednostronne" : "dwustronne"}):</strong> ${formatPLN(tierPrice)}</div>`,
      ];
      ...
      if (tierHintEl) {
        ...
        const discountHint = options.sides === 1 && singleSidedDiscountRate > 0
          ...
          : "";
        tierHintEl.textContent = `Dla ${options.qty} szt użyto ceny ${tierPrice.toFixed(2)} zł (${sideLabel}${discountHint}; format ${options.format}; papier: ${paperVal.replace("_", " ")})`;
      }
