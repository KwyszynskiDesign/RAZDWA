...
import { TierNotFoundError, IncompatibleUnitError, MinimumQuantityError } from "./errors";
 ...
 */
export function findTier(quantity: number, tiers: PriceTier[]): PriceTier | null {
  return (
    tiers.find((tier) => quantity >= tier.min && (tier.max === null || quantity <= tier.max)) ||
    null
  );
}
...
export function calculateBasePrice(
  ...
  if (!tier) {
    ...
    result.errors.push(`Nie znaleziono tier'u dla ilości ${effectiveQuantity} ${priceTable.unit}`);
    return result;
  }
  ...
  if (priceTable.minimumPrice && result.basePrice < priceTable.minimumPrice) {
...
export function formatAllTiers(table: PriceTable): string[] {
  return table.tiers.sort((a, b) => a.min - b.min).map((tier) => formatTierRange(tier, table.unit));
}
