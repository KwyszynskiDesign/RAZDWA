...
export class TierNotFoundError extends RAZDWAError {
  constructor(quantity: number, unit: string, details?: Record<string, unknown>) {
    super(`Nie znaleziono tier'u dla ilości ${quantity} ${unit}`, "TIER_NOT_FOUND", 400, {
      quantity,
      unit,
      ...details,
    });
...
export class ModifierNotFoundError extends RAZDWAError {
  constructor(modifierId: string, available?: string[]) {
    super(`Nieznany modyfikator: ${modifierId}`, "MODIFIER_NOT_FOUND", 400, {
      modifierId,
      available,
    });
...
export class DataNormalizationError extends RAZDWAError {
  constructor(message: string, details?: Record<string, unknown>) {
    super(`Błąd normalizacji danych: ${message}`, "DATA_NORMALIZATION_ERROR", 400, details);
