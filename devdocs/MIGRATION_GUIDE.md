...
export function calculateNowaKategoria(options: NowaKategoriaOptions): CalculationResult {
...
export function calculateProstyProdukt(options: { qty: number }) {
  ...
  const tier = data.tiers.find(
    (t) => options.qty >= t.min && (t.max === null || options.qty <= t.max)
  );
...
```typescript
export function calculateZWariantami(options: { material: string; qty: number }) {
  ...
  const tier = material.tiers.find(
    (t) => options.qty >= t.min && (t.max === null || options.qty <= t.max)
  );
...
```typescript
export function calculateZDoplatami(options: { qty: number; express: boolean; satynowy: boolean }) {
